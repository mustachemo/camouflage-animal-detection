...
with torch.inference_mode(), torch.autocast("cuda", dtype=torch.bfloat16):
    ...
    for image, name in get_input_images():
        ...
        for idx, mask in enumerate(masks):
            print(f"mask.shape: {mask.shape} | mask.dtype: {mask.dtype} | mask.max(): {mask.max()} | mask.min(): {mask.min()}")
            cv2.imwrite(str(OUTPUT_DIR / f"{name}_mask{idx}.jpg"), (mask * 255).astype(np.uint8))